...
$(function() {
  $.get('admin/textsList', function(o) {
    ...
    for (var i = 0; i < o.length; i++) {
      ...
      var text = '<tr><td class="id" scope="row">' + o[i].id +'</td>';
...
$(document).on('click','.delete',function() {
  //On récupère la cellule correspondant au clique et l'id du texte sélectionné.
  var cell = $(this);
  var id = $(this).data('id');
  //Requête Ajax : on appelle la fonction qui prend en paramètre d'url l'id sélectionné.
  $.get("admin/delete/" + id , function(data){
    cell.parent().remove();
    //Envoi d'un message si la requête a abouti..
