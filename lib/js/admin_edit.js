  ...
  $.ajax({
    ...
    //Boucle qui lit toutes les données recues de la BDD et qui les insère dans le tableau.
      for (var i = 0; i < o.length; i++) {
        ...
        var dateslice = o[i].publication_date.split(" ");
        var publiee = new Date(Date.parse(dateslice[0]+"T"+dateslice[1]));

        dateslice = o[i].deferred_date.split(" ");
        var differee = new Date(Date.parse(dateslice[0]+"T"+dateslice[1]));
