  ...
  $.ajax({
    ...
    //Si la requête a fonctionné, on a récupéré les commentaires et on les insère dans le div.
     success: function(data) {
      ...
      //console.log(data);
        var text = '<tr><td class="pseudo_chap">' + data[i].pseudo + '</td>';
...
$(document).on('click','.delete_comChap',function() {
  ...
  $.get(
    ...
    function(data) {
      ...
      $('#delete_commentchap').html("<div class='alert alert-success'>");
      $('#delete_commentchap > .alert-success').html("<button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;").append("</button>");
      $('#delete_commentchap > .alert-success').append("<strong>" + "Le commentaire a bien été supprimé" + "</strong>");
      $('#delete_commentchap > .alert-success').append('</div>');
      //Délai avant la fin du message.
      setTimeout(function() {
        $('#delete_commentchap').html("");
      }, 3000);
    });
  });
