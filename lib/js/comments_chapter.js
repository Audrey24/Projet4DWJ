...
$('#commenterChap').on('click', function() {
  ...
  $.ajax({
    ...
    success: function(data) {
      if (content == "") {
        ...
        $('#commentChp > .alert-danger').append("<strong>" + "Votre commentaire est vide !" + "</strong>" );
        ...
        var text = '<tr><td class="pseudo_chap">' + pseudo + '</td>';
             text += '<td class="content_chap" scope="row">' + encodeURI(content) + '</td>';
...
$(document).on('click','.delete_comChap',function() {
  ...
  $.ajax({
    url : url+ "Current_chapter/delete_commentsChapter/" + id,
    type :"GET",
    async:true,
    success : function(data) {
