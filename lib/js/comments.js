  ...
  $.ajax({
    ...
    success: function(data) {
      if (content == "") {
        ...
        $('#commentMsg > .alert-danger').append("<strong>" + "Votre commentaire est vide !" + "</strong>" );
        ...
        var text = '<tr><td class="pseudo_comments">' + pseudo + '</td>';
             text += '<td class="content_comments" scope="row">' + encodeURI(content) + '</td>';
      ...
      //console.log(data);
        ...
        text += '<td class="date_comments">' + data[i].published_date + '</td>';
          if ((role == "admin") || (role == "moderateur")) {
            ...
            text += '<td><button id="dis'+ data[i].id + '" class="dislike" data-toggle="modal" data-target="#modalComments" data-id ="' + data[i].id + '"><i class="fa fa-exclamation-triangle"></i></button></td></tr>';
          }
        $("#contain_comments").append(text);
      }
    }
...
$(document).on('click','.delete_comments',function() {
  ...
  //Evenement au clique de la classe "dislike".
